...
- **Decision log** (filled in as decisions are made):
  ...
  - 2026-06-11 (Iteration 2, Task 1): Keycap drag uses pointer capture with a 4px threshold to disambiguate click vs drag (the click following a drag is suppressed); position stored as left/top px in localStorage (`devlens-keycap-pos`), clamped to the viewport on restore and on window resize; the slide-out tooltip flips to the right side when the keycap is dragged near the left edge.
